...
```
Score: 0–5 in 0.5 increments. Per-company (batchSize: 1). Uses `AZURE_DEPLOYMENT` (`gpt-5.4`, hardcoded in `src/apis/openai.ts`).
...
| 18 | Company Context Score | Azure OpenAI gpt-5.4, hash-gated |
| 19 | Tooling Match Score | Azure OpenAI gpt-5.4, hash-gated |
| 20 | Intent Signal Score | Azure OpenAI gpt-5.4, hash-gated |
